...
3. **游戏胜负判决**：落子连五、满盘贴目、认输、和棋、超时五条入口。

时序图见 [`docs/时序图.md`](docs/时序图.md)：启动流程、落子与胜负判定、悔棋/超时判负、人机对战。PNG 在 `docs/images/`、`dist/时序图下载.zip`、`dist/wuziqi.zip`。

概要设计说明书（按课程模板）：[`docs/五子棋项目-概要设计说明书.docx`](docs/五子棋项目-概要设计说明书.docx)。
